...
void printUsage() {
	...
	printf("| MODES                                                                       |\n");
	printf("|                                                                             |\n");
	printf("| (# 1) comparison of binary masks (cross comparison)                         |\n");
	printf("| (# 2) usage                                                                 |\n");
	printf("|                                                                             |\n");
	printf("| ARGUMENTS                                                                   |\n");
	printf("|                                                                             |\n");
 ...
 */
void cmp(const Mat a, const Mat b, int& falsePositives, int& falseNegatives, int& incount){
	MatConstIterator_<uchar> pa = a.begin<uchar>();
	MatConstIterator_<uchar> pb = b.begin<uchar>();
	MatConstIterator_<uchar> enda = a.end<uchar>();
	for (;pa<enda; pa++, pb++){
		if (*pa != *pb){
			if (*pa > 0) falsePositives++;
			if (*pb > 0) falseNegatives++;
		}
		if (*pb > 0) incount++;
	}
}
	...
	try {
		...
		if (mode == MODE_MAIN){
			...
			for (vector<string>::iterator infileSmpl = filesSmpl.begin(); infileSmpl != filesSmpl.end(); ++infileSmpl){
				...
				int cnt = img.rows * img.cols;
				string reffile;
				patternFileRename(infilesSmpl,infilesRef,*infileSmpl,reffile);
				Mat imgRef = imread(reffile, CV_LOAD_IMAGE_UNCHANGED);
				CV_Assert(imgRef.data != 0);
				CV_Assert(imgRef.type() == CV_8UC1);
				int fp = 0, fn = 0, pc = 0;
				cmp(img,imgRef,fp,fn,pc);
				//printf("fp %i fn %i pc %i cnt %i\n",fp, fn, pc, cnt);
				double fpr = ((cnt-pc) == 0) ? 0 : ((double) fp) / (cnt-pc);
				double fnr = (pc == 0) ? 0 : ((double)fn) / pc;
				if (!quiet) printf("maskcmp(%s,%s) = (%i FP, %i FN, %f %% FPR,%f %% FNR)\n",(*infileSmpl).c_str(), (reffile).c_str(), fp, fn, 100*fpr, 100*fnr);
				afpr += fpr;
				afnr += fnr;
				if (!outfile.empty() && cfile.is_open()){
					cfile << *infileSmpl << " " << reffile << " " << fpr << " " << fnr << endl;
				}
