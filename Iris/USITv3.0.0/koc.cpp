 ...
 *
 * Calculates distance of iris codes generated by Ko's algorithm
 *
 */
#include "version.h"
#include <cstdio>
...
void printUsage() {
    printVersion();
	printf("+-----------------------------------------------------------------------------+\n");
	printf("| koc - alculates distance of iris codes generated by the Ko et al. algorithm |\n");
	printf("|       (this algorithm does NOT consider masking bits)                       |\n");
	printf("|                                                                             |\n");
	printf("| MODES                                                                       |\n");
	printf("|                                                                             |\n");
...
/**
 * Distance estimation for algorithm of Ko et al.
 * a: sample iris code
 * b: reference iris code
 * codelength8: length of iris-code in bytes
 */
double kdist(const Mat a, const Mat b, const unsigned int codelength8){
	double dist = 0;
	unsigned int code_1[8*codelength8];
	unsigned int code_2[8*codelength8];
	...
	for (;pa<enda; pa++, pb++){
	      ...
	      for (int i = 0; i<8; i++)
	      {
		  code_1[j*8 + 7 - i] = curr_1 % 2;
		  code_2[j*8 + 7 - i] = curr_2 % 2;
		  curr_1 /= 2;
		  curr_2 /= 2;
	      }
	      j++;
	...
	/*
	 * check wheter there are mis-matching upward/downward slopes.
	 * note that downward slopes are within first halfs of iris-code
	 * and upward slopes are with second halfs of iris-codes. 
	 * thus, it is not possible to give a starting and ending block
	 */
	for (int i = 0; i < (int)codelength8*8; i ++)
	{
            if (code_1[i] == 1 && code_2[(i+(codelength8*8)/2)%(codelength8*8)] == 1)
            {
                dist++;
            }
	}
	return dist/codelength8;
		...
		if (mode == MODE_MAIN){
			...
			for (vector<string>::iterator infileSmpl = filesSmpl.begin(); infileSmpl != filesSmpl.end(); ++infileSmpl){
				...
				for (vector<string>::iterator infileRef = filesRef.begin(); infileRef != filesRef.end(); ++infileRef, timing.progress++){
					...
					Mat maskRef;
					maskRef = Mat();
					double score = kdist(imgSmpl[0],imgRef,codeLength);
