...
void printUsage() {
    printVersion();
	printf("+-----------------------------------------------------------------------------+\n");
	printf("| cahtvis - Contrast-adjusted Hough Transform for visible range images        |\n");
	printf("|                                                                             |\n");
	printf("| MODES                                                                       |\n");
	printf("|                                                                             |\n");
    ...
    printf("| -tu  | tuneing    | 6 | N | #1) pixel across the iris (max)                 |\n");
    printf("|      |            |   |   | #2) stddev of above iris diameter for fuzzyness |\n");
    printf("|      |            |   |   | #3) pixel across the pupil (max)                |\n");
    printf("|      |            |   |   | #4) stddev of above pupil  diameter for fuzzyn' |\n");
    printf("|      |            |   |   | #5) pcontrast_min per mil of luminance hist' to |\n");
    printf("|      |            |   |   |                 cut off below the black point   |\n");
    printf("|      |            |   |   | #6) pcontrast_min per mil of luminance hist' to |\n");
    printf("|      |            |   |   |                 cut off above the white point   |\n");
    printf("|      |            |   |   | defaults: 200 0 100 20 40                       |\n");
...
void free_lookup_table(int **lut, int min_r, int max_r)
{
    for (int r = min_r; r <= max_r; r++)
    {
        int ir = r - min_r;
...
void hough_circle(Mat& image, vector<HoughCircle>& res, int min_r, int max_r, int min_cx, int max_cx, int min_cy, int max_cy, int threshold, int hint_r, int hint_x, int hint_y)
{
    ...
    free_lookup_table(lut, min_r, max_r);
}

/*
 * Finds the best Hough circle
 */
int find_best_circle(vector<HoughCircle>& circles, int* x, int* y, int* r, int hint, int hint_x, int hint_y, int hint_r, int hint_weight, double bestdiameter, double bestdiameter_stdev)
{
    int FAIL = -1000;
    int best_accum = FAIL;
    if (! circles.size())
 ...
 * Blacks out pupil
 */
void white_out_circle(Mat& image, int x, int y, int r1)
{
    int w = image.cols;
    int step = image.step;
    int h = image.rows;
...
void scan_edge(Mat& image, int x, int y, int dx, int dy, int n)
{
    ...
    int sx = min(max(x,0), image.cols-1);
    int sy = min(max(y,0), image.rows-1);
    double avg = 0;
    int count = 0;
    for (int i=sx; i != tox; i+=dx)
    {
    	bool mask = false;
    	for (int j=sy; j != toy; j+= dy){
    		if (mask) {
    			data[i + j * step] = 255;
    		}
    		else {
...
int main(int argc, char *argv[])
{
	...
	try {
    	...
    	if (mode == MODE_MAIN){
			...
			cmdCheckOpts(cmd,"-i|-o|-m|-s|-e|-q|-t|-po|-bm|-sr|-lt|-so|-si|-tr|-tc|-l|-tu");
			...
			bool tune = false;
			int pixelsacrossiris = 200;
			int pixelsacrossiris_stdev = 0;
			int pcontrast_min = 20;
			int pcontrast_max = 40;
			int pixelsacrosspupil = 100; // largest circle
			int pixelsacrosspupil_stdev = 0;
			if (cmdGetOpt(cmd,"-tu") != 0){
				cmdCheckOptSize(cmd,"-tu",6);
				pixelsacrossiris = cmdGetParInt(cmd,"-tu",0);
				pixelsacrossiris_stdev = cmdGetParInt(cmd,"-tu",1);
				pixelsacrosspupil = cmdGetParInt(cmd,"-tu",2);
				pixelsacrosspupil_stdev = cmdGetParInt(cmd,"-tu",3);
				pcontrast_min = cmdGetParInt(cmd,"-tu",4);
				pcontrast_max = cmdGetParInt(cmd,"-tu",5);
				tune = true;
			}
			string polarFiles;
			...
			for (vector<string>::iterator inFile = files.begin(); inFile != files.end(); ++inFile, timing.progress++){
				...
				double minArea = 0.02, maxArea = 0.04;
				Mat orig;
				img.copyTo(orig);
				double pixpdiam = 200;
				double pixpdiam_stdev = 0;
				double pixidiam = 200;
				double pixidiam_stdev = 0;
				if (tune) {
					double targetWidth = img.cols * 200. / pixelsacrossiris; 
					double targetHeight = img.rows * 200. / pixelsacrossiris; 
					minArea = pcontrast_min * 0.001;
					maxArea = pcontrast_max * 0.001;
					pixpdiam = 200. * pixelsacrosspupil / pixelsacrossiris; // iris stays at 200
					pixpdiam_stdev = 200. * pixelsacrosspupil_stdev / pixelsacrossiris;
					pixidiam_stdev = 200. * pixelsacrossiris_stdev / pixelsacrossiris;
					cv::resize(img,img,cv::Size(targetWidth,targetHeight),0,0,CV_INTER_CUBIC);
				}
				int width = img.cols;
				int height = img.rows;
				int ix = 0, iy = 0, ir = 0;
				if (!quiet) printf("Finding iris ...\n");
				int IRIS_HOUGH_THRESHOLD = 100;			
				Mat temp(height,width,CV_8UC1);
				img.copyTo(temp);
				adjust_luminance(temp, width * height * minArea, width * height * maxArea);
				...
				invert(temp);
				//PW debug remnants//imwrite("temp.png",temp);
				int hrun = longest_horizontal_run(temp, width * 0.1, 191);
				int vrun = longest_vertical_run(temp, height * 0.1, 191);
				...
				if (tune) {
					min_r = (pixidiam - 2*pixidiam_stdev) < 0 ? 0 : (pixidiam - 2*pixidiam_stdev) / 2;
					max_r = (pixidiam + 2*pixidiam_stdev < max_r) ? (pixidiam + 2*pixidiam_stdev)/2 : max_r;
				}
				...
				hough_circle(accum, circles,min_r, max_r, ax, bx, ay, by, IRIS_HOUGH_THRESHOLD, 0, 0, 0);
				find_best_circle(circles, &ix, &iy, &ir, 1, width / 2, height / 2, 100,0, pixidiam, pixidiam_stdev);
				//cout << "pixidiam: " << pixidiam << "pixidiam_stdev" << pixidiam_stdev << endl;
				...
				int min_pr = 0;
				if (tune) {
					min_pr = (int)((pixpdiam - 2*pixpdiam_stdev) < 0 ? 0 : (pixpdiam - 2*pixpdiam_stdev)/2);
					max_pr = (int)((pixpdiam + 2*pixpdiam_stdev)/2 > ir * 0.9 ? ir* 0.9 : (pixpdiam + 2*pixpdiam_stdev)/2);
				}
				white_out_circle(temp, ix, iy, ir);
				int PUPIL_HOUGH_THRESHOLD = 50;
				adjust_luminance(temp, width * height * 0.01, width * height * 0.2);
				cumulate(temp, accum, 3, 3);
				invert(accum);
				cumulate(accum, temp, 3, 3);
				invert(temp);
				//adjust_luminance(temp, min_pr * min_pr * M_PI, width * height * 0.01);
				//PW debug remnants//imwrite("r1.png",temp);
				
				//threshold_above(temp, 128, 128);
				//imwrite("r2.png",temp);
				
				hrun = longest_horizontal_run(temp, width * 0.25, 127);
				vrun = longest_vertical_run(temp, height * 0.25, 127);
				
				canny(temp, mag, orient, accum, 3, 15, 1, 1);
				remove_streaks(accum, 6, 1, 1, 5, 5, 10, 10, 3, 3);
				bounds_bigger_than_threshold(accum, 0, &ax, &ay, &bx, &by);
				//PW debug remnants//imwrite("accu.png",accum);
				vector<HoughCircle> pupilCircles;
				hough_circle(accum, pupilCircles,min_pr, max_pr, ax, bx, ay, by, PUPIL_HOUGH_THRESHOLD, ir / 1.5, ix, iy);
				find_best_circle(pupilCircles, &px, &py, &pr, 1, ix, iy, (min_pr + max_pr)/2, 10, pixpdiam/2, pixpdiam_stdev);
				...
				//printf("Pupil must be within: %d %d %d %d\n", ax, bx, ay, by);
				//if (bx - ax >= min_pr && by - ay >= min_pr)
				...
				if (tune) {
					pupilCart = (pixelsacrossiris / 200.) * pupilCart; 
					irisCart = (pixelsacrossiris / 200.) * irisCart;
					ix = (int)(ix * (pixelsacrossiris / 200.));
					iy = (int)(iy * (pixelsacrossiris / 200.));
				}
				...
				if (!binmaskFiles.empty()){
					Mat bw(orig.rows,orig.cols,CV_8UC1,Scalar(0));
				...
				rubbersheet(orig, out, pupilCart, irisCart, INTER_LINEAR);
				if (enhance){
