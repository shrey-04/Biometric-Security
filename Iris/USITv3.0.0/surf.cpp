...
void printUsage() {
    printVersion();
	printf("+-----------------------------------------------------------------------------+\n");
	printf("| SURF - feature extraction for enhanced iris textures                        |\n");
	printf("|                                                                             |\n");
	printf("|                                                                             |\n");
	printf("| MODES                                                                       |\n");
	printf("|                                                                             |\n");
    printf("| (# 1) SURF feature extraction from iris textures                            |\n");
    printf("| (# 2) usage                                                                 |\n");
    printf("|                                                                             |\n");
    printf("| ARGUMENTS                                                                   |\n");
    printf("|                                                                             |\n");
...
string featureExtract(string iris_image, string outputfilename) {
	...
	//build Filename for output .surf
	string filename = iris_image;
	int lastindex = filename.find_last_of(".");
	string rawname = filename.substr(0, lastindex);

	string outputFilename = outputfilename;
	outputFilename.append(".surf");

	Mat image = imread(filename, 1);

        Ptr<FeatureDetector> featureDetector = FeatureDetector::create("SURF");
        //set params:
        //featureDetector->set("hessianThreshold", 0);    // Threshold for hessian keypoint detector used in SURF
        //featureDetector->set("hessianThreshold", 0.04); // Number of pyramid octaves the keypoint detector will use
        //featureDetector->set("nOctaveLayers", 10);      // Number of octave layers within each octave
        //featureDetector->set("extended", 3);            // Extended descriptor flag (true - use extended 128-element descriptors; false - use 64-element descriptors)
        //featureDetector->set("extended", 1.6);          // Up-right or rotated features flag (true - do not compute orientation of features; false - compute orientation)
	...
	try {
    	...
    	if (mode == MODE_MAIN){
			...
			for (vector<string>::iterator inFile = files.begin(); inFile != files.end(); ++inFile, timing.progress++){
				...
				if (!quiet) printf("Storing SURF Features to '%s' ...\n", outfile.append(".surf").c_str());
