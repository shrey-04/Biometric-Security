...
void printUsage() {
    printf("+-----------------------------------------------------------------------------+\n");
    printf("| BSIF - Iris-code generation (feature extraction) using the BSIF algorithm   |\n");
    printf("|                                                                             |\n");
    printf("|                                                                             |\n");
    printf("| MODES                                                                       |\n");
    printf("|                                                                             |\n");
    printf("| (# 1) BSIF iris code extraction from iris textures                          |\n");
    printf("| (# 2) usage                                                                 |\n");
    printf("|                                                                             |\n");
    printf("| ARGUMENTS                                                                   |\n");
    printf("|                                                                             |\n");
    ...
    printf("| -f   |            | 1 | N | bsif filter (file name of the filter in the     |\n");
    printf("|      |            |   |   | subdirectory \"texturefilters\"). You can         |\n");
    printf("|      |            |   |   | download this dir with all filters from         |\n");
    printf("|      |            |   |   | \"http://www.ee.oulu.fi/~jkannala/bsif/          |\n");
    printf("|      |            |   |   | bsif_code_and_data.zip\".                        |\n");
    printf("| -o   | outfile    | 1 | N | output iris code image                          |\n");
    printf("| -q   |            | 1 | Y | quiet mode on (off)                             |\n");
    printf("| -t   |            | 1 | Y | time progress on (off)                          |\n");
    printf("| -h   |            | 1 | Y | prints usage                                    |\n");
    printf("+------+------------+---+---+-------------------------------------------------+\n");
    printf("|                                                                             |\n");
 ...
 */
void to_double(double *data, const Mat& mat, const Cell& cell, const int shift) {
    int index = 0;
 ...
 */
emxArray_real_T* parse_img(const Mat& mat, const Cell& cell, const int shift) {
    int size[2] = {cell.endY - cell.startY, cell.endX - cell.startX};
    emxArray_real_T *emx_struct = emxCreateND_real_T(2, &size[0]);

    emx_struct->data = new double[size[0] * size[1]];
 ...
 */
void featureExtract(Mat& code, const Mat& texture, const double filter[], const int filterDims[3], const int shift) {
    ...
    for (int row = 0; row < horizontalCellNumber; row++) {
        for (int col = 0; col < verticalCellNumber; col++, cellIndex++) {

            cell.startX = col * CELL_WIDTH;
            cell.startY = row * CELL_HEIGHT;
            cell.endX = cell.startX + CELL_WIDTH;
            cell.endY = cell.startY + CELL_HEIGHT;

            for (int i = 0; i < histogramSize; i++) histogram[i] = 0; //set all values to 0
            bsif(parse_img(texture, cell, shift), filter, filterDims, histogram, histogramDim); //entry-point for the matlab-implementation
            ...
            for (int histIndex = 0, colIndex = 0; histIndex < histogramSize; histIndex++, colIndex += 2) { //translate 16-Bit integer array to 8-Bit-Image 
                ...
                code.at<uchar>(cellIndex, colIndex) = (histValue & 0xFF00) >> 8;
                code.at<uchar>(cellIndex, colIndex + 1) = (histValue & 0x00FF);
            }
        }
    }
...
/**
 * Extracts the iris texture with 3 diffrent alignments and saves this in one big code map
 * @param code          the big code mat for the extraction data
 * @param texture       the iris texture which should be extracted
 * @param filter        the filter for the bsif extraction algorithm
 * @param filterDims    the dimensions of the filter
 */
void multipleExtract(Mat& code, const Mat& texture, const double filter[], const int filterDims[3]) {
    const int verticalCellNumber = texture.cols / CELL_WIDTH;
    const int horizontalCellNumber = texture.rows / CELL_HEIGHT;
    const int histogrammLength = (int) (pow(2.0, filterDims[2]));
    Mat singleCode(verticalCellNumber*horizontalCellNumber, 2 * histogrammLength, CV_8UC1);
    
    bsif_initialize();

    int counter = 0;
    for (int shift = -DELTA_SHIFT; shift <= DELTA_SHIFT; shift += DELTA_SHIFT, counter++) {
        singleCode.setTo(0);
        featureExtract(singleCode, texture, filter, filterDims, shift);
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (mode == MODE_MAIN) {
            ...
            cmdCheckOpts(cmd, "-i|-o|-m|-q|-t|-f");

            cmdCheckOptExists(cmd, "-i");
            cmdCheckOptSize(cmd, "-i", 1);
            string inFiles = cmdGetPar(cmd, "-i");

            cmdCheckOptExists(cmd, "-f");
            string filter_path = "texturefilters/" + cmdGetPar(cmd, "-f");
            ...
            for (vector<string>::iterator inFile = files.begin(); inFile != files.end(); ++inFile, timing.progress++) {
                ...
                //loading image-------------------------------------------
                Mat texture = imread(*inFile, CV_LOAD_IMAGE_GRAYSCALE);
                CV_Assert(texture.data != 0);
                if (texture.rows != 64 || texture.cols != 512) {
                ...
                int filter_dims[3];
                double *filter_data;
                if (!quiet) printf("Loading filter '%s' ...\n", filter_path.c_str());
                int reading_code = read_filter(filter_dims, &filter_data, filter_path.c_str(), matrix_name);
                if (reading_code < 0) {
                    if (reading_code == -1) CV_Error(CV_StsError, "Could not load filter '" + filter_path + "'");
                    else CV_Error(CV_StsError, "Could not load matrix '" + string(matrix_name) + "'");
                }

                //-----Initialisation Code-Mat------------------
                const int verticalCellNumber = texture.cols / CELL_WIDTH;
                const int horizontalCellNumber = texture.rows / CELL_HEIGHT;
                const int histogrammLength = (int) (pow(2.0, filter_dims[2]));
                Mat code(verticalCellNumber*horizontalCellNumber*3, 2 * histogrammLength, CV_8UC1); //multiple extract with 3 (-8,0,8)

                //feature extraction---------------------------------------                
                if (!quiet) printf("Creating %d x %d iris-code ...\n", histogrammLength, verticalCellNumber * horizontalCellNumber*3);
                multipleExtract(code, texture, filter_data, filter_dims);
