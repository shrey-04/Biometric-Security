 ...
 *
 * Calculates distance of iris codes generated by BSIF algorithm
...
void printUsage() {
	printf("+-----------------------------------------------------------------------------+\n");
	printf("| bsifc - calculates distance of iris codes generated by the BSIF             |\n");
	printf("| algorithm                                                                   |\n");
	printf("|                                                                             |\n");
	printf("| MODES                                                                       |\n");
	printf("|                                                                             |\n");
	...
	printf("| -i *.png *.png -o compare.txt -q -t                                         |\n");
	printf("|                                                                             |\n");
	printf("| AUTHOR                                                                      |\n");
	printf("|                                                                             |\n");
...
/**
 * Binary distance extimation for BSIF algorithm
 * @param a             Mat a for comparison
 * @param b             Mat b for comparison
 * @param offsetA       row offset of Mat a (for alignment)
 * @param offsetB       row offset of Mat b (for alignment)
 * @param rowLength     the number of rows (cells) which should be compared (for alignment)
 * @return              the binary distance of the compared cells
 */
double minimal_shift_dist(const Mat a, const Mat b, const int rowOffsetA, const int rowOffsetB, const int rowLength){    
    double *cellDistances = new double[rowLength];
    
    for(int rowIndex = 0; rowIndex < rowLength; rowIndex++){ // cell/row
        
        cellDistances[rowIndex] = 0;
        
        for(int colIndex = 0; colIndex < a.cols; colIndex+=2){ // histogram values
            
            uint16_t valueA = *a.ptr<uchar>(rowIndex+rowOffsetA, colIndex) << 8;
            valueA |= *a.ptr<uchar>(rowIndex+rowOffsetA, colIndex+1);
            double histValueA = valueA / MAX_UNSIGNED_16_BIT_VALUE;
            
            uint16_t valueB = *b.ptr<uchar>(rowIndex+rowOffsetB, colIndex) << 8; 
            valueB |= *b.ptr<uchar>(rowIndex+rowOffsetB, colIndex+1);
            double histValueB = valueB / MAX_UNSIGNED_16_BIT_VALUE;
                        
            if(histValueA != 0 || histValueB != 0){                 
                cellDistances[rowIndex] += ((histValueA-histValueB)*(histValueA-histValueB))/(histValueA+histValueB); 
            }
        }
        
        cellDistances[rowIndex] /= a.cols;
    }    
    
    double averageCellDistance = 0;
    for(int rowIndex = 0; rowIndex < rowLength; rowIndex++) averageCellDistance += cellDistances[rowIndex];
    return (1000*averageCellDistance)/(double)(rowLength);  
    
    delete[] cellDistances;
}

/**
 * Calculates the minimal difference of two bit maps
 * @param a     the fist bit map
 * @param b     the second bit map
 * @return      the minimal distance of the compares
 */
double multiple_dist(const Mat a, const Mat b){    
    double a_b_left = minimal_shift_dist(a, b, 1.0/3.0*a.rows, 0, a.rows/3.0);
    double a_b_zero = minimal_shift_dist(a, b, 1.0/3.0*a.rows, 1.0/3.0*b.rows, a.rows/3.0);
    double a_b_right = minimal_shift_dist(a, b, 1.0/3.0*a.rows, 2.0/3.0*b.rows, a.rows/3.0);
    return min(a_b_left, min(a_b_zero, a_b_right));
}
	...
	try {
		...
		if (mode == MODE_MAIN){
			...
			for (vector<string>::iterator infileSmpl = filesSmpl.begin(); infileSmpl != filesSmpl.end(); ++infileSmpl){
				vector<Mat> imgSmpl;
				Mat img = imread(*infileSmpl, CV_LOAD_IMAGE_GRAYSCALE);
				...
				for (vector<string>::iterator infileRef = filesRef.begin(); infileRef != filesRef.end(); ++infileRef, timing.progress++){
					...
					CV_Assert(imgRef.size() == codeSize);					
                                        double score = multiple_dist(imgSmpl[0],imgRef);
