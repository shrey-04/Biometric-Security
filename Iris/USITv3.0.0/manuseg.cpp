...
void printUsage() {
    ...
    cout << "| -so  | scale fac. | 1 | N | scale the area of the outer ellipse (def.=1.0)  |" << endl;
    cout << "| -si  | scale fac. | 1 | N | scale the area of the inner ellipse (def.=1.0)  |" << endl;
    cout << "| -tr  | translate  | 1 | N | horizontal translation of ellipses (def.=0.0)   |" << endl;
    cout << "|      |            |   |   | the factor is by iris radius (-1 would be a     |" << endl;
    cout << "|      |            |   |   | translation by iris radius to the left)         |" << endl;
    cout << "| -trl |            | 1 | N | translate eyelid as well as iris centers        |" << endl;
    cout << "| -tc  | translate  | 1 | N | horizontal translation of unrolling center.0)   |" << endl;
    cout << "|      |            |   |   | works as per translate (but is based on pupil   |" << endl;
    cout << "|      |            |   |   | radius, moust be in -1 to 1) (default=0.0).     |" << endl;
    cout << "| -l   | logfile    | 1 | N | log parameters for unrolling to this file.      |" << endl;
    cout << "+------+------------+---+---+-------------------------------------------------+" << endl;
    cout << "|                                                                             |" << endl;
    cout << "| AUTHOR                                                                      |" << endl;
    cout << "|                                                                             |" << endl;
...
// sample an ellipse
void ellipsesample(const RotatedRect& ellipse, vector<Point2f>& poly, const int samples=720) {
	...
	const float thetaoffset = 2 * M_PI / samples;
	float theta = thetaoffset/2.;
	for (; theta<2*M_PI; theta += thetaoffset){
        ...
        poly.push_back(Point2f(
		    centerX + cosAlpha * s - sinAlpha * t, // x coordiante
		    centerY + sinAlpha * s + cosAlpha * t  // y coordinate
            ));
	}
}

bool intersectline( const Point2f& s1, const Point2f& e1, const Point2f& s2, const Point2f& e2, Point2f& result){
    Point2f  x = s2 - s1;
    Point2f d1 = e1 - s1;
    Point2f d2 = e2 - s2;
...
/** 
 * intersects two line segments, if the intersection point is outside of the two segments returns false, 
 * otherwise true.
 * The intersection is returned in result.
 */
bool intersectlinesegment( const Point2f& s1, const Point2f& e1, const Point2f& s2, const Point2f& e2, Point2f& result){
    if( intersectline( s1, e1, s2, e2, result)){
        Point2f d1 = e1 - s1;
        Point2f d2 = e2 - s2;
        float l1 = (result.x - s1.x)/d1.x;
        float l2 = (result.x - s2.x)/d2.x;
...
void loadPoints(string filename, vector<Point2f>& points) {
    ...
    if( coordfile.is_open() ) {
        cout <<"File is open: "<<filename<<endl;
    }
	while (coordfile.is_open()  && !coordfile.eof()){
		string line;
 ...
 */
void ellipse2Cart(Mat& cart, const RotatedRect& ellipse, const float offset_x = 0) {
    ...
    if( abs(offset_x) < 1e-10){ //original 
        double alpha = (ellipse.angle)*M_PI/180; // inclination angle of the ellipse
        if (alpha > M_PI) alpha -= M_PI; // normalize alpha to [0,M_PI]
        const double cosAlpha = cos(alpha);
        const double sinAlpha = sin(alpha);
    ...
    } else { // use poly scanned version with line intersects
        /*
        string outname="ellipse2Cart";
        outname+=(char)('0'+idcount);
        outname+=".txt";
        printf("outname: %s\n", outname.c_str());
        ofstream ofs(outname.c_str(), ios_base::out);
        printf("Polycase, offset is %12.10f, scan written in %s\n", offset_x, outname.c_str());
        */
        vector<Point2f> poly;
        ellipsesample( ellipse,  poly, width*2);
        float radius=(ellA+ellB)*2;
        Point2f s1( centerX+offset_x, centerY);
        Point2f e1;
        Point2f result;
        for (int x=0; x<width; x++, d++, theta += thetaoffset){
            e1 = Point2f(
                    s1.x + radius*cos(theta),
                    s1.y + radius*sin(theta) );
            result = Point2f(centerX,centerY);
            for( vector<int>::size_type i = 0; i != poly.size(); i++ ){
                vector<int>::size_type ii = i+1;
                if( ii == poly.size() ) ii = 0;
                if( intersectlinesegment(s1, e1, poly[i], poly[ii], result)) break;
            }
            *d = result.x;
            d++;
            *d = result.y;
            //ofs << result.x << " "<<result.y <<endl;
        }
        //ofs.close();
    }
}
...
int main(int argc, char *argv[])
{
	...
	try {
    	...
    	if (mode == MODE_MAIN) {
			...
			cmdCheckOpts(cmd,"-i|-c|-o|-m|-s|-sr|-srl|-roi|-e|-q|-t|-so|-si|-tr|-trl|-tc|-l");
            ...
            if (cmdGetOpt(cmd,"-so") != 0){
                ...
                scale_outer = cmdGetParFloat(cmd, "-so", 0);
                /* 
                 * A = pi a b
                 * xA = pi sa sb = s²ab pi = s²A
                 * => x = sqrt(s)
                 */
                if ( scale_outer <=0){
                    cerr << "scale_outer factor (-so) <=0, this will result in an error" << endl;
                }
                scale_outer = sqrt(scale_outer);
            }
            float scale_inner = 1;
            if (cmdGetOpt(cmd,"-si") != 0){
                cmdCheckOptSize(cmd, "-si", 1);
                scale_inner = cmdGetParFloat(cmd, "-si", 0);
                if ( scale_inner <=0){
                    cerr << "scale_inner factor (-si) <=0, this will result in an error" << endl;
                }
                scale_inner = sqrt(scale_inner);
            }
            float translate = 0;
			...
			bool translate_lid = false;
			if (cmdGetOpt(cmd,"-trl") != 0){
				cmdCheckOptSize(cmd,"-trl",0);
				translate_lid = true;
			}
			ofstream logFile;
			...
			for (vector<string>::iterator infile = files.begin(); infile != files.end(); ++infile, timing.progress++){
                ...
                if ( 0 == innerPoints.size()){
                    cout << "EE Too few points, skipping this input" << endl;
                    continue;
                }
                ...
                int translate_from_outer =  translate * outer.size.width/2.;
                outer.size.width *= scale_outer;
                outer.size.height *= scale_outer;
                outer.center.x += translate_from_outer;
                //scale inner
				RotatedRect inner = fitEllipse(innerPoints);
                float translate_from_inner =  translate_unroll * inner.size.width/2.;
                inner.size.width*= scale_inner;
                inner.size.height*= scale_inner;
                inner.center.x += translate_from_outer;
                //unrolling
                if( abs(translate_from_inner) > 1e-10){
                    printf("Inner RotatedRect at (%f,%f) size (%f,%f) angle (%f) offset_x (%f)\n", 
                            inner.center.x, inner.center.y,
                            inner.size.width, inner.size.height,
                            inner.angle, translate_from_inner);
                    printf("Outer RotatedRect at (%f,%f) size (%f,%f) angle (%f) offset_x (%f)\n", 
                            outer.center.x, outer.center.y,
                            outer.size.width, outer.size.height,
                            outer.angle, translate_from_inner);
                } else{
                    printf("Inner RotatedRect at (%f,%f) size (%f,%f) angle (%f)\n", 
                            inner.center.x, inner.center.y,
                            inner.size.width, inner.size.height,
                            inner.angle);
                    printf("Outer RotatedRect at (%f,%f) size (%f,%f) angle (%f)\n", 
                            outer.center.x, outer.center.y,
                            outer.size.width, outer.size.height,
                            outer.angle);
                }
                ...
                ellipse2Cart(outerBoundary,outer, translate_from_inner);
                ellipse2Cart(innerBoundary,inner, translate_from_inner);
                //prep offset for eyelid translate
                Point eyelid_offset = Point(0,0);
                if (translate_lid){
                    eyelid_offset.x = translate_from_outer;
                }
                if (!q) cout << "Eyelid offset: " << eyelid_offset << endl;
				...
				if (!maskfiles.empty() || !segresfiles.empty()|| !segreslfiles.empty()){
					...
					fillPoly(bw,upPoints,&nupper,1,Scalar(0,0,0), 8, 0, eyelid_offset);
					fillPoly(bw,downPoints,&nlower,1,Scalar(0,0,0), 8, 0, eyelid_offset);
				...
				if (!roifiles.empty()){
					...
					fillPoly(roi,upPoints,&nupper,1,Scalar(128), 8, 0, eyelid_offset);
					fillPoly(roi,downPoints,&nlower,1,Scalar(128), 8, 0, eyelid_offset);
