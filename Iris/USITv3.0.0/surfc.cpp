 ...
 *
 * Calculates similarity of two sets of SURF keypoints
 *
 */
#include "version.h"
#include <cstdio>
...
#define OPT_PARAM_X 18
#define OPT_PARAM_Y 3
...
void printUsage() {
    printVersion();
	printf( "+-----------------------------------------------------------------------------+\n");
	printf( "| SURFC - Calculates similarity of two sets of SURF keypoints                 |\n");
	printf( "|                                                                             |\n");
	printf( "|                                                                             |\n");
	printf( "| MODES                                                                       |\n");
	printf( "|                                                                             |\n");
	...
	printf( "+------+------------+---+---+-------------------------------------------------+\n");
	printf( "| -i   | infile1    | 1 | N | source/reference .surf files (use * as wildcard,|\n");
	...
	printf( "|                                                                             |\n");
	printf( "| -i s1.surf s2.surf -o compare.txt                                           |\n");
	printf( "| -i *.surf *.surf -s -o compare.txt -q -t                                    |\n");
	printf( "|                                                                             |\n");
	printf( "| AUTHOR                                                                      |\n");
	printf( "|                                                                             |\n");
...
/**
 * Read the keypoints from the .surf file
 */
...
/**
 * Read the feature descriptors from the .surf file
 */
...
/**
 * Read the image from the .surf file
 ...
 * a: sample surf features
 * b: reference surf features
 */

float surfCompare(string reference_file, string probe_file) {
		...
		if (mode == MODE_MAIN) {
			...
			for (vector<string>::iterator infileSmpl = filesSmpl.begin(); infileSmpl != filesSmpl.end(); ++infileSmpl) {
				for (vector<string>::iterator infileRef = filesRef.begin(); infileRef != filesRef.end(); ++infileRef, timing.progress++) {
					double score = surfCompare((*infileSmpl).c_str(),(*infileRef).c_str());
					if (!quiet) 
