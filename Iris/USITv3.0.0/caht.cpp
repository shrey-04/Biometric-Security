...
void printUsage() {
    ...
    printf("| -noscale |        |   |   | Disable automatic downscaleing to 320x280.      |\n");
    printf("|      |            |   |   | Downscale happens if image larger than 480x420. |\n");
    printf("+------+------------+---+---+-------------------------------------------------+\n");
    printf("|                                                                             |\n");
 ...
 */
vector<std::pair<int,int> > scan_edge(Mat& image, int x, int y, int dx, int dy, int n)
{
    vector<std::pair<int,int> > ret;
    //sanity check:
    if( image.rows == 0 or image.cols == 0) return ret;
	int step = image.step;
    uchar * data = image.data;
...
void mask_lids(const Mat& orig_image, Mat& mask, int px, int py, int pr, int ix, int iy, int ir)
{
    ...
    //low
    auto points = scan_edge(edges, 0, iy-yl, 1, -1, ir);
    auto points2 = scan_edge(edges, edges.cols - 1, iy-yl, -1, -1, ir);
    points.insert(points.end(), points2.begin(), points2.end());
    //move to correct posititon
    for( auto it = points.begin(); it != points.end(); ++it){
        it->first += xl;
        it->second += yl;
    }
    /*** PWILD debug messages??
    for( auto it = points.begin(); it != points.end(); ++it){
        if( it->first == ix or
            it->first == ix-ir or
            it->first == ix+ir){
            printf("LOW POINTS SELECT: %d,%d\n", it->first, it->second);
        } else{
            printf("L unselect       : %d,%d\n", it->first, it->second);
        }
    }
    */
    //high
    points = scan_edge(edges, 0, iy-yl, 1, 1, ir);
    points2 = scan_edge(edges, edges.cols - 1, iy-yl, -1, 1, ir);
    //move to correct posititon
    points.insert(points.end(), points2.begin(), points2.end());
    for( auto it = points.begin(); it != points.end(); ++it){
        it->first += xl;
    ...
    for( auto it = points.begin(); it != points.end(); ++it){
        if( it->first == ix or
            it->first == ix-ir or
            it->first == ix+ir){
            printf("HIGH POINTS SELECT: %d,%d\n", it->first, it->second);
        } else{
            printf("H unselect        : %d,%d\n", it->first, it->second);
        }
    }
    */
    mask.setTo(255);
...
int main(int argc, char *argv[])
{
	...
	try {
    	...
    	if (mode == MODE_MAIN){
			...
			cmdCheckOpts(cmd,"-i|-o|-m|-s|-e|-q|-t|-po|-bm|-sr|-lt|-so|-si|-tr|-tc|-l|-tu|-noscale");
			...
			bool downscale = true;
			if (cmdGetOpt(cmd,"-noscale") != 0){
				cmdCheckOptSize(cmd,"-noscale",0);
				downscale = false;
			}
			int lt = 1;
			...
			for (vector<string>::iterator inFile = files.begin(); inFile != files.end(); ++inFile, timing.progress++){
                ...
                if( downscale){
                    Size isize = img.size();
                    //casia4 for reference is 320x280
                    printf("downscale check: %dx%d ...",img.size().width, img.size().height);
                    if( isize.width > 480 or isize.height > 420){
                        Size osize(320,280);
                        if((int)isize.height*(320./isize.width) < 280)
                            osize.height =(int)isize.height*(320./isize.width);
                        else
                            osize.width = (int)isize.width*(280./isize.height);
                        printf(" scaling to %dx%d\n",osize.width, osize.height);
                        img.copyTo(imgCol);
                        resize(imgCol, img, osize, 0, 0, CV_INTER_LANCZOS4);
                    } 
				...
				if (tune) {
					min_r = (pixpdiam - 2*pixpdiam_stdev) < 0 ? 0 : (pixpdiam - 2*pixpdiam_stdev);
					max_r = (pixpdiam + 2*pixpdiam_stdev > max_r) ? pixpdiam + 2*pixpdiam_stdev : max_r;
				}
				...
				find_best_circle(circles, &px, &py, &pr, 1, width / 2, height / 2, height / 2,0, pixpdiam, pixpdiam_stdev);
				...
				int min_ir = pr * 1.5;
				if (tune) {
					min_ir = (int)((pixidiam - 2*pixidiam_stdev) < pr * 1.1 ? pr* 1.1 : (pixidiam - 2*pixidiam_stdev));
				}
				...
				/* point to e.g. let it lie outside the pupil */
				{
					...
					find_best_circle(irisCircles, &ix, &iy, &ir, 1, px, py, pr, 10, pixidiam, pixidiam_stdev);
				}
