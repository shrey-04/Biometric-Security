 ...
 *
 * Calculates distance of iris codes generated by Monro's algorithm
 *
 */
#include "version.h"
#include <cstdio>
...
void printUsage() {
    printVersion();
	printf("+-----------------------------------------------------------------------------+\n");
	printf("| dctc - alculates distance of iris codes generated by the Monro et al.       |\n");
...
struct ring *read_dct_file(unsigned char *code, int length)
{
    struct ring *all;
    ...
    int curr, i, j, count = 0;

    struct node *left12node = (node*)malloc(sizeof(struct node));
    left12node->data =  (unsigned char*)malloc(sizeof(unsigned char) * partsize);
    struct node *left8node = (node*)malloc(sizeof(struct node));
    left8node->data =  (unsigned char*)malloc(sizeof(unsigned char) * partsize);
    struct node *left4node = (node*)malloc(sizeof(struct node));
    left4node->data =  (unsigned char*)malloc(sizeof(unsigned char) * partsize);
    struct node *orig = (node*)malloc(sizeof(struct node));
    orig->data =        (unsigned char*)malloc(sizeof(unsigned char) * partsize);
    struct node *right4node = (node*)malloc(sizeof(struct node));
    right4node->data =  (unsigned char*)malloc(sizeof(unsigned char) * partsize);
    struct node *right8node = (node*)malloc(sizeof(struct node));
    right8node->data =  (unsigned char*)malloc(sizeof(unsigned char) * partsize);
    struct node *right12node = (node*)malloc(sizeof(struct node));
    right12node->data =  (unsigned char*)malloc(sizeof(unsigned char) * partsize);


    /*ring for iterate*/
    all = (ring*)malloc(sizeof(struct ring));
    all->first = left12node;
    all->size = 7;
    left12node->next = left8node;
    left8node->next = left4node;
    left4node->next = orig;
    orig->next = right4node;
    right4node->next = right8node;
    right8node->next = right12node;
    right12node->next = left12node;

    actualnode = all->first;
    binaryCode = actualnode->data;
    
    while(count < 7)
    {
        binaryCode = actualnode->data;
        for (i = 0; i < partsize / 8; i++)
        {
            curr =code[count*partsize / 8 + i];
            
            /* create the binary code */
            for (j = 0; j < 8; j++)
            {
                binaryCode[i * 8 + 7 - j] = curr % 2;
                curr /= 2;
            }
        }
    ...
    all->dctsize = 3;
    all->sizepercoeff = partsize / all->dctsize; //TOCHECK
    return all;
}


double podhd(unsigned char *first, unsigned char *second, int sizepercoeff, int dctsize)
{
    int j, i;
    int *amount = (int*)malloc(sizeof(int) * dctsize);
    double *dists = (double*)malloc(sizeof(double) * dctsize);
    int c = 0;
    for(j = 0; j < dctsize; j++)
    {
        c = j * sizepercoeff;
        ...
        dists[j] = amount[j] / (double)sizepercoeff;
    }
    double distance = 1.0;
    ...
    double root = NROOT(dctsize, distance);
    free(amount);
    free(dists);
    return root;
}

double compute_mininal_poshd(struct ring *first, struct ring *second)
{
    double hd = 1.0;
    double temphd = 1.0;
    struct node *actualfirst, *actualsecond;
    unsigned char *firstdata, *seconddata;
    int i = 0, j = 0;
    
    actualfirst = first->first;
    actualsecond = second->first;
    while(i < 7)
    {
        firstdata = actualfirst->data;
        j = 0;
        while(j < 7)
        {
            seconddata = actualsecond->data;
            temphd = podhd(firstdata, seconddata, first->sizepercoeff, first->dctsize);
            if(temphd < hd)
            {
                hd = temphd;
            }
            j++;
            actualsecond = actualsecond->next;
        }
        i++;
...
void freering(struct ring *r)
{
    struct node *actualnode = r->first, *next;
    int c = 0;
    while(c < 7)
    {
        free(actualnode->data);
...
/**
 * Distance estimation for algorithm of Monro et al.
 * a: sample iris code
 * b: reference iris code
 * codelength8: length of iris-code in bytes
 */
double dctdist(const Mat a, const Mat b, const unsigned int codelength8){
	...
	first  = read_dct_file(code1, codelength8);
	second = read_dct_file(code2, codelength8);
	

	distance = compute_mininal_poshd(first, second);
	freering(first);
	freering(second);

	return distance/*/(codelength8*8)*/;
}
		...
		if (mode == MODE_MAIN){
			...
			for (vector<string>::iterator infileSmpl = filesSmpl.begin(); infileSmpl != filesSmpl.end(); ++infileSmpl){
				...
				for (vector<string>::iterator infileRef = filesRef.begin(); infileRef != filesRef.end(); ++infileRef, timing.progress++){
					...
					Mat maskRef;
					maskRef = Mat();
					double score = dctdist(imgSmpl[0],imgRef,codeLength);
