 ...
 *
 * Calculates distance of iris codes generated by uniform LBP algorithm
 *
 */
#include "version.h"
#include <cstdio>
...
void printUsage() {
    printVersion();
	printf("+-----------------------------------------------------------------------------+\n");
	printf("| lbpc - calculates distance of iris codes generated by the uniform LBP       |\n");
	printf("| algorithm                                                                   |\n");
	printf("|                                                                             |\n");
	printf("| MODES                                                                       |\n");
	printf("|                                                                             |\n");
	...
	printf("| -h   |            | 2 | N | prints usage                                    |\n");
        printf("| -c   | w x h      | 2 | Y | cell size of the lbp feature-extractor          |\n");
	printf("+------+------------+---+---+-------------------------------------------------+\n");
	printf("|                                                                             |\n");
 ...
 */
double chi_square_dist(const Mat a, const Mat b, const int rowOffsetA, const int rowOffsetB, const int rowLength){    
    ...
    for(int rowIndex = 0; rowIndex < rowLength; rowIndex++){ // cell/row
        ...
        for(int colIndex = 0; colIndex < a.cols; colIndex+=2){ // histogram values
            ...
            if(valueA != 0 || valueB != 0){                 
                cellDistances[rowIndex] += ((valueA-valueB)*(valueA-valueB))/(valueA+valueB); 
            }
        }
    ...
    return averageCellDistance/(2*rowLength);  
    
    delete[] cellDistances;
}

/**
 * Calculates the minimal difference of two mat files by the chi square algorithm
 * @param a     the fist bit map
 * @param b     the second bit map
 * @return      the minimal distance of the compares
 */
double min_dist(const Mat a, const Mat b){    
    double a_b_left = chi_square_dist(a, b, 1.0/3.0*a.rows, 0, a.rows/3.0);
    double a_b_zero = chi_square_dist(a, b, 1.0/3.0*a.rows, 1.0/3.0*b.rows, a.rows/3.0);
    double a_b_right = chi_square_dist(a, b, 1.0/3.0*a.rows, 2.0/3.0*b.rows, a.rows/3.0);
		...
		if (mode == MODE_MAIN){
			...
			for (vector<string>::iterator infileSmpl = filesSmpl.begin(); infileSmpl != filesSmpl.end(); ++infileSmpl){
				...
				for (vector<string>::iterator infileRef = filesRef.begin(); infileRef != filesRef.end(); ++infileRef, timing.progress++){
					...
					Mat maskRef;
					maskRef = Mat();                                        
					double score = min_dist(imgSmpl[0],imgRef);
